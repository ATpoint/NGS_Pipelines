...
usage(){
  echo '
## Creation of bedGraphs starting from Bam files ##
...
Details: * Option --extend is simply -fs in bedtools genomecov. If --atacseq is set then this can be
         ...
         * Option --normalize works together with --peaks. The peak list will be used to create a count
           ...
           The $4 of the bedGraphs is then divided by the per-library normalization factor.
...
Tools=(Rscript featureCounts bedtools bgzip mawk)
...
#------------------------------------------------------------------------------------------------------------------------
#/ Function to take the BAM files, a peak list, making a count matrix with featureCounts,
#/ then stream this to the R script to get size factors.
...
function Bam2Bw {
  ...
  | sort -k1,1 -k2,2n --parallel="${sortthreads}" -S "${sortmem}" \
  | bgzip > "${Basename}""${suffix}".bedGraph
...
if [[ -f OK ]]; then
  ...
  echo '[ERROR] --normalize was set but did not finish properly.'
  echo 'Either sample not found'
  exit 1
